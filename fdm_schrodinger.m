...
% 9 different potential energy configurations are included in this 
...
asym_double_square_well = false;
sym_double_square_well = false;
quartic_fn = true;
...
if half_harmonic_oscillator
  for i = 1:0.5*number_of_data_points
    potential(i,i) = 5 + potential(i,i);
    end
...
if asym_double_square_well
    for i = 1:0.20*number_of_data_points
        potential(i,i) = 1 + potential(i,i);
    end
    for i = 0.20*number_of_data_points+1:0.40*number_of_data_points
        potential(i,i) = 0.1 + potential(i,i);
    end
    for i = 0.4*number_of_data_points+1:0.5*number_of_data_points
        potential(i,i) = 0.5 + potential(i,i);
    end
    for i = 0.5*number_of_data_points+1:0.8*number_of_data_points
        potential(i,i) = 0.1 + potential(i,i);
    end
    for i = 0.8*number_of_data_points+1:number_of_data_points
        potential(i,i) = 1 + potential(i,i);
    end
end

if sym_double_square_well

    for i = 1:0.20*number_of_data_points
        potential(i,i) = 1 + potential(i,i);
    end
    for i = 0.20*number_of_data_points+1:0.45*number_of_data_points
        potential(i,i) = 0.1 + potential(i,i);
    end
    for i = 0.45*number_of_data_points+1:0.55*number_of_data_points
        potential(i,i) = 0.5 + potential(i,i);
    end
    for i = 0.55*number_of_data_points+1:0.8*number_of_data_points
        potential(i,i) = 0.1 + potential(i,i);
    end
    for i = 0.8*number_of_data_points+1:number_of_data_points
        potential(i,i) = 1 + potential(i,i);
...
if quartic_fn
  for i = 1:number_of_data_points
    potential(i,i) = 0.00000001*(0.05*(i-number_of_data_points/2)^4 - 20*(i-number_of_data_points/2)^3 - (i-number_of_data_points/2)^2 + (i-number_of_data_points/2)) + potential(i,i);
     if potential(i,i) >= 5;
            potential(i,i) = 5;
     end
  if potential(i,i) <= -5;
...
third_excited_state = eigenvectors(:,4) + eigenvalues(4,4)*ones(number_of_data_points,1);
fourth_excited_state = eigenvectors(:,5) + eigenvalues(5,5)*ones(number_of_data_points,1);
fifth_excited_state = eigenvectors(:,6) + eigenvalues(6,6)*ones(number_of_data_points,1);
sixth_excited_state = eigenvectors(:,7) + eigenvalues(7,7)*ones(number_of_data_points,1);

area(x_positions, diag(potential))
hold on
plot(x_positions, ground_state_wavefunction,  'LineWidth', 2)
hold on
plot(x_positions, first_excited_state,  'LineWidth', 2)
hold on
plot(x_positions, second_excited_state, 'LineWidth', 2)
hold on
plot(x_positions, third_excited_state,  'LineWidth', 2)
hold on
plot(x_positions, fourth_excited_state,  'LineWidth', 2)
hold on
plot(x_positions, fifth_excited_state,  'LineWidth', 2)
hold on
plot(x_positions, sixth_excited_state,  'LineWidth', 2)

legend('Potential Energy', 'Ground State', 'First Excited State', 'Second Excited State', 'Third Excited State', 'Fourth Excited State', 'Fifth Excited State', 'Sixth Excited State')
xlabel('Position')
ylabel('Energy (hbar and mass set to one)')
title('Wavefunction vs. Position for the First Four Energy Levels')
